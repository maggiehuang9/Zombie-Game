...
{
	if (timesAttacked == 0)//has not been attacked yet
	{
		timesAttacked++;
		//does not move back if on the edge
...
void Player::moveOrAttack(int dir)
{
	...
	{
	case UP:
		if (m_row < 2) break;  // do nothing, cannot move out of the arena
		...
		break;
	case DOWN:
		if (m_row + 1 > m_arena->rows()) break;  // do nothing, cannot move out of the arena
		if (m_arena->numZombiesAt(m_row + 1, m_col) > 0)
		{
			m_arena->attackZombieAt(m_row + 1, m_col, dir);
		}
		...
		break;
	case LEFT:
		if (m_col < 2) break;  // do nothing, cannot move out of the arena
		...
		break;
	case RIGHT:
		if (m_col + 1 > m_arena->cols()) break;  // do nothing, cannot move out of the arena
		if (m_arena->numZombiesAt(m_row, m_col + 1) > 0)
		{
			m_arena->attackZombieAt(m_row, m_col + 1, dir);
		}
...
int Arena::numZombiesAt(int r, int c) const
{
	...
	for (int i = 0; i < m_nZombies; i++)      
	{
		if (m_zombies[i]->row() == r && m_zombies[i]->col() == c)  //zombie found at location
		{
			numZombies++;
		}
	}
...
void Arena::display() const
{
	...
	for (r = 0; r < rows(); r++)
		for (c = 0; c < cols(); c++)
		{
			int nZombiesAt_r_c = numZombiesAt(r + 1, c + 1); // using r+1 and c+1 is because row and col number start from 1, not zero
			if (nZombiesAt_r_c > 0)
			{
				if (nZombiesAt_r_c == 1)
					grid[r][c] = 'Z';
				else if (nZombiesAt_r_c > 1 && nZombiesAt_r_c < 9)
					grid[r][c] = '0' + nZombiesAt_r_c;
				else
					grid[r][c] = '9';
			}
		}
...
{
	if (m_nZombies > MAXZOMBIES) //max number of zombies
		return false;
...
bool Arena::attackZombieAt(int r, int c, int dir)
{
	if (numZombiesAt(r, c) == 0)
		return false;
	int nZombies = -1;  // set to negative value
	for (int i = 0; i < m_nZombies; i++)
	{
	...
	if (nZombies < 0) return false; // zombie didn't die yet
	for (int j = nZombies; j < m_nZombies - 1; j++)
	{
		m_zombies[j] = m_zombies[j + 1];
	}

	m_nZombies--;  //decrement number of zombies
	return true;
}
...
bool Arena::moveZombies()
{
	...
	{
		m_zombies[k]->move();
		if (m_zombies[k]->col() == m_player->col() && m_zombies[k]->row() == m_player->row())//zombie and player are in same position
			m_player->setDead();
	}
